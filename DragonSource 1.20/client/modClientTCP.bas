...
Sub TcpInit()
    ...
    If FileExist("config.ini") Then
        ...
        frmMain.chkbubblebar.Value = ReadINI("CONFIG", "SpeechBubbles", FileName)
        frmMain.chknpcbar.Value = ReadINI("CONFIG", "NpcBar", FileName)
        frmMain.chknpcname.Value = ReadINI("CONFIG", "NPCName", FileName)
        frmMain.chkplayerbar.Value = ReadINI("CONFIG", "PlayerBar", FileName)
        frmMain.chkplayername.Value = ReadINI("CONFIG", "PlayerName", FileName)
        
        frmMain.chkplayerdamage.Value = ReadINI("CONFIG", "NPCDamage", FileName)
        frmMain.chknpcdamage.Value = ReadINI("CONFIG", "PlayerDamage", FileName)
        frmMapEditor.optMapGrid.Value = ReadINI("CONFIG", "MapGrid", FileName)
        frmMain.chkmusic.Value = ReadINI("CONFIG", "Music", FileName)
        frmMain.chksound.Value = ReadINI("CONFIG", "Sound", FileName)
    Else
        IP = "127.0.0.1"
        Port = 4000
    ...
    frmMain.Socket.RemotePort = Val(Port)
End Sub
...
Dim Start As Long
    ...
    frmMain.Socket.GetData Buffer, vbString, DataLength
...
Dim z As Long
    ...
    If LCase(Parse(0)) = "maxinfo" Then
        ...
        frmMain.Caption = Trim(GAME_NAME)
        App.Title = GAME_NAME
    ...
    If LCase(Parse(0)) = "playerhp" Then
        ...
        If GetPlayerMaxHP(MyIndex) > 0 Then
            'Debug.Print frmMain.shpHP.Width
                frmMain.shpHP.FillColor = RGB(208, 11, 0)
                frmMain.shpHP.Width = (((GetPlayerHP(MyIndex) / 126) / (GetPlayerMaxHP(MyIndex) / 126)) * 126)
                frmMain.lblHP.Caption = GetPlayerHP(MyIndex) & " / " & GetPlayerMaxHP(MyIndex)
        End If
        Exit Sub
    End If
    ...
    If LCase(Parse(0)) = "playermp" Then
        ...
        'Debug.Print frmMain.shpMP.Width
                frmMain.shpMP.FillColor = RGB(208, 11, 0)
                frmMain.shpMP.Width = (((GetPlayerMP(MyIndex) / 126) / (GetPlayerMaxMP(MyIndex) / 126)) * 126)
                frmMain.lblMP.Caption = GetPlayerMP(MyIndex) & " / " & GetPlayerMaxMP(MyIndex)
        End If
        Exit Sub
    End If
    ...
    'If LCase(Parse(0)) = "playersp" Then
       ' Player(MyIndex).MaxSP = Val(Parse(1))
        ...
        'If GetPlayerMaxSP(MyIndex) > 0 Then
            'frmMain.lblSP.Caption = Int(GetPlayerSP(MyIndex) / GetPlayerMaxSP(MyIndex) * 100) & "%"
        'End If
        'Exit Sub
    'End If
    ...
    If (LCase(Parse(0)) = "playerstatspacket") Then
        ...
        If SubStr > 0 Then
            frmMain.lblSTR.Caption = Val(Parse(1)) - SubStr & " (+" & SubStr & ")"
        Else
            frmMain.lblSTR.Caption = Val(Parse(1))
        End If
        If SubDef > 0 Then
            frmMain.lblDEF.Caption = Val(Parse(2)) - SubDef & " (+" & SubDef & ")"
        Else
            frmMain.lblDEF.Caption = Val(Parse(2))
        End If
        If SubMagi > 0 Then
            frmMain.lblMAGI.Caption = Val(Parse(4)) - SubMagi & " (+" & SubMagi & ")"
        Else
            frmMain.lblMAGI.Caption = Val(Parse(4))
        End If
        If SubSpeed > 0 Then
            frmMain.lblSPEED.Caption = Val(Parse(3)) - SubSpeed & " (+" & SubSpeed & ")"
        Else
            frmMain.lblSPEED.Caption = Val(Parse(3))
        End If
        frmMain.lblEXP.Caption = Val(Parse(6))
        Debug.Print frmMain.shpTNL.Width
        frmMain.shpTNL.Width = (((Val(Parse(6)) / 100) / (Val(Parse(5)) / 100)) * 100)
        frmMain.lblLevel.Caption = "" & Val(Parse(7))
    ...
    If (LCase(Parse(0)) = "trade") Then
        ...
        frmTrade.Show vbModeless, frmMain
        Exit Sub
    End If
    ...
    If (LCase(Parse(0)) = "spells") Then
        ...
        frmMain.picPlayerSpells.Visible = True
        frmMain.lstSpells.Clear
        
        ' Put spells known in player record
        For i = 1 To MAX_PLAYER_SPELLS
            Player(MyIndex).Spell(i) = Val(Parse(i))
            If Player(MyIndex).Spell(i) <> 0 Then
                frmMain.lstSpells.AddItem i & ": " & Trim(Spell(Player(MyIndex).Spell(i)).Name)
            Else
                frmMain.lstSpells.AddItem "<free spells slot>"
            End If
        Next i
        
        frmMain.lstSpells.ListIndex = 0
    End If
    ...
    frmMain.lstOnline.Clear
    
        n = 2
        z = Val(Parse(1))
        For X = n To (z + 1)
            frmMain.lstOnline.AddItem Trim(Parse(n))
            n = n + 2
        Next X
        Exit Sub
    End If
    ...
    If LCase(Parse(0)) = "pptrading" Then
        ...
        frmPlayerTrade.Show vbModeless, frmMain
        Exit Sub
    End If
...
' :::::::::::::::::::::::::
    If LCase(Parse(0)) = "ppchatting" Then
        ...
        frmPlayerChat.Show vbModeless, frmMain
        Exit Sub
    End If
...
' :::::::::::::::::::::::::::::
    ...
    If LCase(Parse(0)) = "flashevent" Then
        If LCase(Mid(Trim(Parse(1)), 1, 7)) = "http://" Then
            ...
            'frmFlash.Show vbModeless, frmMain
